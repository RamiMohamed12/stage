...
class CunningDocumentScannerPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        if (call.method == "getPictures") {
            val noOfPages = call.argument<Int>("noOfPages") ?: 1;
            val isGalleryImportAllowed = call.argument<Boolean>("isGalleryImportAllowed") ?: false;
            this.pendingResult = result
            val language = call.argument<String>("lang") ?: "fr"
            startScan(noOfPages, isGalleryImportAllowed, language)
        } else {
     ...
     */
    private fun startScan(noOfPages: Int, isGalleryImportAllowed: Boolean, language: String ) {
