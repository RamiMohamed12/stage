...
class DecujusService {
  ...
  Future<Map<String, dynamic>> verifyDecujusByPensionNumberAndAgencyId(String pensionNumber, int agencyId) async {
    final token = await _tokenService.getToken();
    ...
    final url = Uri.parse('${ApiEndpoints.baseUrl}/decujus/pension/$pensionNumber/agency/$agencyId');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return {
          ...
          'exists': true,
          'data': data,
          'message': 'Decujus trouvé avec succès pour cette agence'
        };
      } else if (response.statusCode == 404) {
        return {
          'success': true,
          'exists': false,
          'data': null,
          'message': 'Aucun decujus trouvé avec ce numéro de pension pour cette agence'
        };
      } else if (response.statusCode == 400) {
        ...
        return {
          ...
          'exists': false,
          'data': null,
          'message': errorData['message'] ?? 'Paramètres invalides'
        };
      } else if (response.statusCode == 401 || response.statusCode == 403) {
        throw Exception('Non autorisé: Token invalide');
      } else {
