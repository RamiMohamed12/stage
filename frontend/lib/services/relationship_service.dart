...
class RelationshipService {
  ...
  Future<List<Document>> getRequiredDocumentsForRelationship(int relationshipId) async {
    final token = await _tokenService.getToken();
    if (token == null) {
      throw Exception('Token not found. Please login again.');
    }
    ...
    final response = await http.get(
      Uri.parse('${ApiEndpoints.relationship}/$relationshipId/required-documents'),
      headers: {
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': 'Bearer $token',
      },
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to load required documents: ${response.statusCode}');
    }
  }
}
